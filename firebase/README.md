...
M8 진급·보관(`archived`, 교사의 `students`/`progress` 학년·반 패치)을 쓰려면 이 파일을 다시 게시하세요.
   포스트잇 메모(`notes` 컬렉션, 2026-09-22 #99)를 쓰려면 이 파일을 다시 게시하세요. 게시 전에는 메모 만들기가 '규칙' 오류로 거부됩니다. 규칙이 이전 버전이면 명단 보관·진급 일괄 반영이 거부됩니다.
   기기 간 편집 잠금(`editLocks` 컬렉션)도 이 파일을 다시 게시해야 동작합니다. 게시 전에는 잠금 읽기·쓰기가 조용히 실패하고(콘솔에 경고만 남음) 편집기는 기존처럼 잠금 없이 동작합니다.
...
| `archived` | 불리언 (졸업 보관) | `true` | 선택 |
| `archivedAt` | 시각 | (서버 시각) | 선택 |
...
졸업 코호트는 교사 운영 화면에서 `archived=true`로 보관하거나 명단 한 줄만 지웁니다. 학습 기록은 지우지 않습니다.
진급 때 교사가 `students`·`progress`의 학년·반만 맞추려면 **이 규칙 파일을 다시 게시**해야 합니다.
...
editLocks/{uid}                       기기 간 편집 잠금(30초 하트비트, 2분 무응답이면 만료)
```
