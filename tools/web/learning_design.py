...
'widgets':('위젯의 역할을 맞춰 보세요',['Entry / QLineEdit｜입력','Button / QPushButton｜실행 요청','Label / QLabel｜짧은 출력','Text / QPlainTextEdit｜여러 줄'],['역할','tkinter','PySide6'],[['최상위 창','Tk','QWidget / QMainWindow'],['그룹','Frame','QWidget + Layout'],['입력 읽기','entry.get()','edit.text()'],['표시 바꾸기','label.config(text=...)','label.setText(...)'],['독립 선택','Checkbutton + BooleanVar','QCheckBox.isChecked()'],['그룹 중 하나','Radiobutton + StringVar','QRadioButton / QButtonGroup'],['목록·그리기','Listbox / Canvas','QListWidget / QGraphicsView']]),
...
'kivy':('본편 개념을 Kivy로 옮기세요',['App 클래스 작성','build()에서 Layout 구성','bind로 이벤트 연결','run()'],['역할','tkinter','Kivy'],[['앱·창','Tk()','App + build()'],['설명 글','Label','Label'],['한 줄 입력','Entry','TextInput(multiline=False)'],['여러 줄','Text','TextInput(multiline=True)'],['버튼 클릭','command=','bind(on_press=)'],['배치','pack / grid','BoxLayout / GridLayout'],['이벤트 루프','mainloop()','App.run()']]),
}
...
def visual(u,l,prefix="../../"):
 ...
 diagram=flow(steps)
 if l['id'] in ['random','thirdparty','ui','libraries','widgets','wx','kivy','ml-methods','ml-libraries']:
...
2:['58쪽 4번의 사용자 정의 예외 표현은 보기의 GUI 사례와 맞지 않아 인터페이스 분류로 재작성했습니다.','GUI의 문법 확인은 실제 창·버튼 동작 검증이 아닙니다. PC 시연을 별도로 확인합니다.','PySide6는 교과서 tkinter 이후 확장 학습입니다. 확장 사용 자체를 별도 평가 요건으로 추가하지 않습니다.','wxPython 부록은 본편 tkinter/PySide6를 대체하지 않습니다. 브라우저에는 wx가 없어 코드·캡처로 가르치고, 실제 창은 PC에서 확인합니다.','Kivy 부록도 본편을 대체하지 않습니다. Pyodide는 Kivy를 실행하지 못하므로 코드·캡처로 가르치고, PC에서 pip install kivy 후 확인합니다.'],
...
def teacher_pages(web,layout,units,notes):
 ...
 home+='</div><h2>자료 사용 순서</h2>'+flow(['교사용 요약에서 목표·오개념 확인','수업 화면으로 구조 설명','학생용 실습 링크로 전환','결과·설명·저널 확인'])+'<section class="teacher-note"><h2>차시와 평가의 기준</h2><p>평가 항목은 제공된 2026학년도 2학기 운영 계획 _g에 근거합니다. 아래 시간 배분은 50분 수업 운영 제안이며 확정 시수나 평가 기준을 변경하지 않습니다. 기존 학생 화면의 일률적인 “6차시” 표기는 제거했습니다.</p><p>성취기준·세부 점수 부여와 실제 제출 경로는 학교의 확정 문서에 따릅니다. 웹 연습 완료·자동 검사는 공식 성적이 아닙니다.</p></section><p><a href="admin.html">수업 관리 · 명단 →</a> · <a href="ops.html">입학년도 운영 →</a> · <a href="board.html">반 현황 보드 →</a> · <a href="session.html">수업 세션 →</a> · <a href="assignments.html">과제와 제출 →</a> · <a href="../index.html">학생용 대문으로 →</a></p></main>'
