...
TIPS = {
 ...
 2: {
  ...
  'kivy': ('idea','같은 앱을 터치 UI 도구로 다시 조립해 봐요.','App → build() → run(). 브라우저에는 Kivy가 없으니 코드와 실행 화면으로 역할을 맞추세요.'),
...
def lesson_tip(u,id,prefix):
 mood,title,text=TIPS.get(u,{}).get(id,('idea','입력부터 결과까지, 한 단계씩 연결해 봐요.','먼저 흐름도에서 데이터가 바뀌는 곳을 찾고, 코드의 같은 부분을 바꾸어 결과를 비교하세요.'))
 return card(prefix,mood,title,text)
def hero():
 return '<aside class="pai-hero" aria-labelledby="pai-hello"><div class="pai-hero-copy"><span class="pai-label">우리 과목의 학습 친구</span><h2 id="pai-hello">안녕하세요, <em>파이</em>예요!</h2><p>같이 생각하고, 오류를 찾고,<br>작은 성공을 쌓아 가요.</p></div>'+image('','welcome','eager','pai-hero-image')+'<div class="pai-speech" aria-live="off"><b id="pai-mood-title">반가워요</b><p id="pai-mood-description">'+MOODS['welcome'][2]+'</p></div><div class="pai-mood-buttons" role="group" aria-label="파이의 표정과 학습 안내">'+''.join(f'<button data-pai-preview="{mood}" data-pai-description="{esc(info[2],quote=True)}" data-pai-alt="{esc(info[1],quote=True)}" aria-pressed="{str(mood=="welcome").lower()}">{info[0]}</button>' for mood,info in MOODS.items())+'</div><button class="pai-auto" id="pai-auto" aria-pressed="true">자동 전환 일시정지</button><p class="pai-auto-hint">6초마다 표정과 안내가 바뀌어요.</p><a class="pai-about" href="mascot.html">파이를 소개합니다 · 캐릭터 내려받기 ↗</a></aside>'
def intro_page():
...
def runtime_guide(u):
 ...
 if u>=3: text='데이터나 조건 하나를 바꾸고 결과를 비교하세요. 웹 실행과 PC 실습 표시를 먼저 확인해요.'
