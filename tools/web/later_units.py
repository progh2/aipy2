...
units.update({3:[],4:[]})
def topic(u,id,title,pages,lead,paragraphs,steps,headers,rows,ids=(),tasks=(),api=(),history='',youtube=None,screenshot=None):
 lesson(u,id,title,pages,lead,paragraphs,ids,tasks,api=api,history=history,youtube=youtube,screenshot=screenshot)
 units[u][-1]['visual']=(title+' · 한눈에 보기',steps,headers,rows)
def example(id,title,src,packages='',mode='web',files=None,checks='',note='',api=(),history='',youtube=None,screenshot=None):
 data={'main.py':src,**(files or {})}
 if packages:data['requirements.txt']=packages.replace(' ',',') .replace(',','\n')+'\n'
 ex(id,title,data,mode=mode,checks=checks,note=note,api=api,history=history,youtube=youtube,screenshot=screenshot)
