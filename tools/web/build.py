...
session_body = '''<main id="main" class="admin-page teacher-manage"><p class="eyebrow">TEACHER / SESSION</p><h1>수업 세션</h1><p class="lead">따라가기 세션은 반 하나에 하나입니다. 여기서 고른 반이 세션 문서 ID가 됩니다.</p><section class="admin-card"><h2>지금 수업하는 반</h2><p class="teacher-class-banner"><strong data-class-label>반을 선택하세요</strong></p><p class="small">세션 키: <code data-class-id></code> · Firestore <code>sessions/{학년}-{반}</code></p></section><section class="admin-card teacher-placeholder"><h2>준비 중</h2><p>세션 시작·종료와 초점 이동은 다음 단계(M5)에서 이 반에 붙습니다. 이 화면에서는 반만 고를 수 있습니다.</p></section><p><a href="board.html">현황 보드로 →</a></p></main>'''
...
(WEB/'teacher/session.html').write_text(teacher_manage('session','수업 세션',session_body))
