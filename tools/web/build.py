...
# 페이지 공통 스크립트는 layout()에 둡니다(app/auth/follow/sync/understanding/help).
# 단원 페이지에만 필요한 모듈은 여기. 학습 기록 동기화(sync.js)는 대문·교사 화면도
# 같은 localStorage를 쓰므로 layout()에 한 줄만 넣고, 생성기 훅을 늘리지 않습니다.
# 이해도·도움 요청 UI는 생성 HTML을 고치지 않고 JS가 .completion / #lab / #practice 에 붙입니다.
...
(WEB/'teacher/board.html').write_text(teacher_manage('board','반 현황 보드',board_body,'<script type="module" src="../assets/teacher-board.js?v=1"></script>'))
(WEB/'teacher/session.html').write_text(teacher_manage('session','수업 세션',session_body,'<script type="module" src="../assets/teacher-session.js?v=1"></script>'))
dump(WEB/'data/catalog.json',{
 ...
 'examples':{k:{'title':v['title']} for k,v in examples.items()}
})
