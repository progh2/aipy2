...
# 같은 localStorage를 쓰므로 layout()에 한 줄만 넣고, 생성기 훅을 늘리지 않습니다.
# 이해도·도움 요청·과제 제출 UI는 생성 HTML을 고치지 않고 JS가 .completion / #lab / #practice 에 붙입니다.
def unit_scripts(prefix,unit):
 extras=''
 if unit:
  extras+=f'<script type="module" src="{prefix}assets/teacher-focus.js?v=7"></script>'
 return extras
...
def practice(u,topic=''):
 ...
 note='이 주제와 연결된 문제만 보여 줍니다. 단원 전체 문제는 단원 안내 페이지에서 이어서 풀 수 있습니다.' if topic else '실행 검사는 결과와 입력 조건을 확인합니다. 빈칸은 대표 답안과 비교하며, 다른 표현은 해설을 보고 판단하세요. GUI 코드는 PC 실행 점검까지 마치세요.'
 attr=f' data-topic-filter="{esc(topic)}"' if topic else ''
 extra=f'<p class="small"><a href="index.html#practice">단원 전체 {sum(q["unit"]==u for q in questions)}문제로 →</a></p>' if topic else ''
 return f'''<section id="practice" class="section exercise-section"{attr}><p class="eyebrow">PRACTICE / RETRY / UNDERSTAND</p><h2>{label} <span class="count">{count}</span></h2><p>{note}</p>{extra}{mascot.card("../../","thinking","먼저 예상하고, 힌트는 한 단계씩 열어 보세요.","답을 확인한 다음에는 조건을 바꾸어 다시 풀어 보세요. 다른 올바른 풀이도 있을 수 있어요.","practice")}<div class="filter-row"><label>유형<select id="question-kind"><option value="">모든 유형</option></select></label><label>상태<select id="question-status"><option value="">전체</option><option value="todo">미완료</option><option value="retry">다시 풀기</option><option value="done">완료</option></select></label><label>검색<input id="question-search" type="search" placeholder="예: 임포트, 메모장"></label></div><div class="question-progress"><p id="question-summary" aria-live="polite"></p><progress id="question-progress" value="0" max="{count or 1}" aria-label="연습 문제 완료 진행률"></progress></div><div id="questions"></div></section>'''
...
def gallery(u,open_on_page=True):
 ...
 for id,title,version,desc,example in gallery_items:
  # 편집기가 예제 전용 페이지로 옮겨졌으므로(#101) 갤러리도 그 페이지로 바로 연결한다.
  open_btn=f'<a class="button" data-example="{example}" href="{textbook.example_file(example)}">코드 열기 ↗</a>'
  s+=f'<article class="gallery-card"><a href="../../assets/screenshots/{id}.png" target="_blank" rel="noopener"><img src="../../assets/screenshots/{id}.png" loading="lazy" alt="{title} 인사 앱 실제 실행 화면 · 이름 입력창과 인사 및 초기화 버튼"></a><div><h3>{title}</h3><p>교과서 {textbook.BOOK[u][1]}쪽 · 2개 중단원</p><p>{desc}</p><small>{version}</small>{open_btn}</div></article>'
...
for u,lessons in units.items():
 ...
 body+=practice(u)+journal_block(u,prefix)+'<div class="actions end-nav">'+(f'<a class="button primary" href="../unit0{u+1}/index.html">다음: {design.META[u+1][0]} →</a>' if u<4 else '<a class="button" href="../../index.html">전체 단원으로 →</a>')+'</div></main></div>'
 ...
 for i,l in enumerate(lessons):
  ...
  topic_rail=textbook.toc(u,lessons)+ (f'<a href="#{l["id"]}-lab">실습 카드</a>' if l['examples'] else '')+rail_extra+'<a href="#practice">연습 문제</a><a href="index.html#journal">학습 저널</a><a href="index.html">단원 안내</a>'
  ...
  jump='<nav class="lesson-jump" aria-label="이 주제 바로가기"><a href="#'+l['id']+'"><b>1</b> 설명</a>'+(f'<a href="#{l["id"]}-lab"><b>2</b> 실습</a>' if l['examples'] else '')+f'<a href="#practice"><b>{3 if l["examples"] else 2}</b> 문제 {qcount}개</a></nav>'
  # 설명 → 실습 → 그 주제 문제 순으로 붙이고, 갤러리·체험 같은 보조 자료는 뒤로 보낸다(#91)
  topic_body+=jump+lesson_section(u,l,i,len(lessons),prefix,True)+practice(u,l['id'])+extras+nav_links+'</main></div>'
  ...
  for ei,eid in enumerate(l['examples']):
   ...
   ex_rail=textbook.toc(u,lessons)+f'<div class="rail-examples"><p class="eyebrow">이 소단원 실습</p><ul>{ex_rail_items}</ul></div>'+f'<a href="{textbook.topic_file(l["id"])}">소단원 설명으로</a><a href="index.html#practice">연습 문제</a><a href="index.html#journal">학습 저널</a><a href="index.html">단원 안내</a>'
   ...
   ex_nav+=f'<a class="button primary" href="{textbook.example_file(next_e)}">다음 실습: {esc(examples[next_e]["title"])} →</a>' if next_e else f'<a class="button primary" href="{textbook.topic_file(l["id"])}#practice">이 소단원 문제 풀기 →</a>'
   ex_nav+='</div>'
...
for u,ls in units.items(): sourcebody+=textbook.toc(u,ls,f'units/unit0{u}/')
...
def teacher_manage(page, title, inner, extra_scripts=''):
 body = f'<div id="teacher-shell" data-teacher-page="{page}"></div>{inner}'
 scripts = '<script type="module" src="../assets/teacher-shell.js?v=2"></script>' + extra_scripts
 return layout(title, body, '../', 0, scripts)

admin_body = '''<main id="main" class="admin-page teacher-manage"><p class="eyebrow">TEACHER / ROSTER</p><h1>학생 명단 관리</h1><p class="lead">학생의 학번·입학년도를 미리 등록합니다. 학년·반·번호는 학번(학년1·반1~2·번호2자리)에서 자동 계산됩니다. 학생은 이 명단의 값만 받아 가며 스스로 바꿀 수 없습니다. 진급·졸업 정리는 <a href="ops.html">운영</a>에서 합니다.</p><p class="teacher-context">지금 보는 반: <strong data-class-label>반을 선택하세요</strong></p><section class="admin-card" id="admin-gate">권한을 확인합니다…</section><div id="admin-tools" hidden><section class="admin-card"><h2>명단 일괄 등록</h2><p class="small">CSV 열: <code>email,studentId,admissionYear,name,grade,classroom,number</code> · 학년·반·번호는 비우면 학번에서 자동 계산됩니다(예: 2314 → 2학년 3반 14번). 머리글은 한글(<code>이메일,학번,입학년도,이름,학년,반,번호</code>)도 인식합니다. 일괄 등록은 모든 반을 대상으로 하며, 아래 목록만 선택한 반으로 걸러집니다.</p><label class="file-button">CSV 파일 선택<input type="file" id="roster-file" accept=".csv,text/csv,text/plain"></label><label>또는 직접 붙여넣기<textarea id="roster-text" rows="8" spellcheck="false" placeholder="s2314@e-mirim.hs.kr,2314,2025,홍길동,,,"></textarea></label><div class="actions"><button id="roster-check">검사하기</button><button class="primary" id="roster-apply" disabled>명단에 반영</button><button id="roster-sample">예시 채우기</button></div><p id="roster-summary" aria-live="polite"></p><div id="roster-preview" class="admin-table"></div></section><section class="admin-card"><h2>현재 명단 <span id="roster-count" class="pill"></span></h2><p class="small" id="roster-scope">수업할 반을 위에서 선택하면 그 반 명단만 보입니다.</p><div class="actions"><button id="roster-load">불러오기</button><button id="roster-export">CSV 내보내기</button><button id="roster-clean-legacy" title="학번이 4자리가 아닌 학생을 명단에서 지웁니다">5자리 학번 정리</button></div><div id="roster-list" class="admin-table"></div></section><section class="admin-card"><h2>미배정 학생</h2><p class="small">로그인했지만 명단에 없는 학교 계정입니다. 학기 중 전학·누락을 여기서 확인해 추가하세요. 이 목록은 반이 없으므로 반 선택과 관계없이 보여 줍니다.</p><div class="actions"><button id="unassigned-load">확인하기</button></div><div id="unassigned-list" class="admin-table"></div></section></div><p><a href="index.html">교사용 수업 요약으로 →</a></p></main>'''
board_body = '''<main id="main" class="admin-page teacher-manage"><p class="eyebrow">TEACHER / BOARD</p><h1>반 현황 보드</h1><p class="lead">이 반 학생의 접속·완료·막힌 곳·도움 요청을 한눈에 봐요.</p><section class="admin-card"><h2>지금 수업하는 반</h2><p class="teacher-class-banner"><strong data-class-label>반을 선택하세요</strong> <span class="small" data-class-id></span></p><p class="small">여러 반이 동시에 수업해도 이 반만 보여요.</p></section><section class="admin-card" id="roster-board"><h2>학생 카드</h2><p class="small" id="roster-note">학번순입니다. 카드를 누르면 상세를 봅니다.</p><p id="roster-counts" class="teacher-class-banner">학생 0 · 접속 0 · 도움 0</p><div class="actions"><button type="button" id="board-export">CSV 내보내기</button><p class="small">학번·완료·정답·도움만 내려받아요. 성적용은 아니에요.</p><button type="button" id="heatmap-clear" hidden>주제 필터 해제</button></div><div id="roster-list"></div></section><section class="admin-card" id="heatmap-board"><h2>히트맵</h2><p class="small" id="heatmap-note">주제별로 완료·이해도 색을 봐요.</p><div id="heatmap-legend" class="heatmap-legend"><span data-tone="done">완료</span><span data-tone="understood">이해했어요</span><span data-tone="somewhat">조금 어려워요</span><span data-tone="hard">어려워요</span><span data-tone="empty">미완료</span></div><div id="heatmap-wrap" class="heatmap-wrap"></div></section><section class="admin-card" id="question-board"><h2>문항 분석</h2><p class="small" id="question-note">시도·정답률·평균 시도입니다. 정답률이 낮은 문항이 위로 옵니다.</p><p id="question-totals" class="teacher-class-banner">시도 0 · 정답 0</p><div id="question-list" class="admin-table"></div></section><section class="admin-card" id="understanding-board"><h2>이해도 신호</h2><p class="small" id="understanding-note">이해도 신호와 소단원별 학생 의견을 봐요.</p><p id="understanding-counts" class="teacher-class-banner">어려워요 0 · 조금 어려워요 0 · 이해했어요 0</p><div id="understanding-list" class="admin-rows"></div></section><section class="admin-card" id="help-board"><h2>도움 요청</h2><p class="small" id="help-note">요청 시각순입니다. 처리하면 해결로 표시됩니다. 학생이 보내는 신호예요. 점수·출결에는 안 반영돼요.</p><p id="help-counts" class="teacher-class-banner">대기 0</p><div id="help-list" class="admin-rows"></div></section><aside id="student-detail" class="student-detail" hidden aria-hidden="true" role="dialog" aria-modal="true" aria-labelledby="student-detail-title"><div class="student-detail-panel"><button type="button" id="student-detail-close">닫기</button><div id="student-detail-body"></div></div></aside><p><a href="admin.html">명단 관리로 →</a></p></main>'''
session_body = '''<main id="main" class="admin-page teacher-manage"><p class="eyebrow">TEACHER / SESSION</p><h1>수업 세션</h1><p class="lead">이 반 학생 화면을 같이 따라가게 할 수 있어요. 위에서 고른 반으로만 세션이 열려요.</p><section class="admin-card"><h2>지금 수업하는 반</h2><p class="teacher-class-banner"><strong data-class-label>반을 선택하세요</strong></p></section><section class="admin-card" id="session-panel"><h2>세션</h2><p id="session-status" class="small">반을 선택하면 세션 상태가 표시됩니다.</p><div class="actions"><button class="primary" id="session-start" type="button" disabled>세션 시작</button><button id="session-end" type="button" disabled>세션 종료</button></div></section><section class="admin-card" id="focus-panel"><h2>초점</h2><p class="small">같은 페이지는 스크롤로, 다른 단원은 로컬 저장 후 이동합니다. 시선 모으기는 배너만 보여 주고 화면을 옮기지 않습니다.</p><label>페이지<select id="focus-page"></select></label><label>주제<select id="focus-topic"></select></label><label>예제<select id="focus-example"></select></label><div class="actions"><button class="primary" id="focus-send" type="button" disabled>초점 보내기</button><button id="attention-send" type="button" disabled>시선 모으기</button></div><p id="focus-note" class="small"></p></section><section class="admin-card" id="together-panel"><h2>함께 풀기</h2><p class="small">선택형 문제 하나를 초점으로 보내면, 반 응답 분포를 이름 없이 막대로 봅니다.</p><label>단원<select id="together-unit"><option value="">모든 단원</option><option value="1">1단원</option><option value="2">2단원</option><option value="3">3단원</option><option value="4">4단원</option></select></label><label>문제<select id="together-question"></select></label><div class="actions"><button class="primary" id="together-send" type="button" disabled>이 문제로 함께 풀기</button><button id="together-refresh" type="button" disabled>응답 새로고침</button></div><p id="together-note" class="small"></p><div id="together-bars"></div></section><section class="admin-card" id="lesson-report"><h2>수업 리포트</h2><p class="small">선택한 반과 최근 세션을 기준으로 다음 차시 도입에 쓸 요약을 만듭니다.</p><div class="actions"><button id="report-refresh" type="button">요약 만들기</button></div><div id="report-summary"><p class="small">반을 고른 뒤 요약을 만들면 다음 차시 도입에 쓸 수 있어요.</p></div></section><section class="admin-card"><h2>따라가기</h2><p id="presence-counts" class="teacher-class-banner">따라오는 중 0 / 따로 보는 중 0</p><p class="small">최근 3분 안에 신호를 보낸 학생만 셉니다. 인원수만 표시합니다.</p></section><p><a href="board.html">현황 보드로 →</a></p></main>'''
(WEB/'teacher').mkdir(exist_ok=True)
...
(WEB/'teacher/admin.html').write_text(teacher_manage('roster','학생 명단 관리',admin_body,'<script type="module" src="../assets/admin.js?v=4"></script>'))
(WEB/'teacher/ops.html').write_text(teacher_manage('ops','입학년도 운영',ops_body,'<script type="module" src="../assets/ops.js?v=3"></script>'))
(WEB/'teacher/board.html').write_text(teacher_manage('board','반 현황 보드',board_body,'<script type="module" src="../assets/teacher-board.js?v=6"></script>'))
(WEB/'teacher/session.html').write_text(teacher_manage('session','수업 세션',session_body,'<script type="module" src="../assets/teacher-session.js?v=3"></script>'))
...
dump(WEB/'data/catalog.json',{
 ...
 'topics':{f'units/unit0{u}/index.html':[{'id':l['id'],'title':l['title'],'examples':list(l['examples']),'href':f'units/unit0{u}/{l["id"]}.html'} for l in lessons] for u,lessons in units.items()},
  ...
  **({'answer':q['answer']} if q['kind'] in ('선택','빈칸','예측') and q.get('answer') not in (None,'') else {})
 } for q in questions]
})
