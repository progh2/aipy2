...
board_body = '''<main id="main" class="admin-page teacher-manage"><p class="eyebrow">TEACHER / BOARD</p><h1>반 현황 보드</h1><p class="lead">선택한 반의 이해도 신호와 도움 요청을 실시간으로 봅니다. 히트맵·문항 분석은 다음 단계에서 붙습니다.</p><section class="admin-card"><h2>지금 수업하는 반</h2><p class="teacher-class-banner"><strong data-class-label>반을 선택하세요</strong> <span class="small" data-class-id></span></p><p class="small">조회·집계는 이 반만 대상으로 합니다. 여러 반이 동시에 수업해도 데이터가 섞이지 않습니다.</p></section><section class="admin-card" id="understanding-board"><h2>이해도 신호</h2><p class="small" id="understanding-note">이 신호는 평가에 반영되지 않습니다. 막힌 학생을 찾기 위한 표시입니다.</p><p id="understanding-counts" class="teacher-class-banner">어려워요 0 · 조금 어려워요 0 · 이해했어요 0</p><div id="understanding-list" class="admin-rows"></div></section><section class="admin-card" id="help-board"><h2>도움 요청</h2><p class="small" id="help-note">요청 시각순입니다. 처리하면 해결로 표시됩니다. 평가에 반영되지 않습니다.</p><p id="help-counts" class="teacher-class-banner">대기 0</p><div id="help-list" class="admin-rows"></div></section><section class="admin-card teacher-placeholder"><h2>준비 중</h2><p>학생 카드, 주제×학생 히트맵, 문항 분석은 다음 단계에서 이 반(<code data-class-id></code>)을 기준으로 붙습니다.</p></section><p><a href="admin.html">명단 관리로 →</a></p></main>'''
