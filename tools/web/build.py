...
def nav(prefix): return f'''<a class="skip" href="#main">본문으로 건너뛰기</a><header class="top"><a class="brand" href="{prefix}index.html">{mascot.image(prefix,"welcome","eager","pai-brand")}인공지능 파이썬 실무</a><nav aria-label="주 메뉴"><a href="{prefix}before-you-start.html">시작하기 전에</a><a href="{prefix}units/unit01/index.html">01 모듈</a><a href="{prefix}units/unit02/index.html">02 GUI</a><a href="{prefix}units/unit03/index.html">03 ML</a><a href="{prefix}units/unit04/index.html">04 CV</a></nav></header>'''
...
for u,title,desc,tags in [(1,'모듈과 패키지 활용','코드를 나누고 연결하면서 재사용 가능한 나만의 도구를 만듭니다.',['다중 파일 실행','표준 라이브러리','60문제']),(2,'GUI 프로그래밍','tkinter로 원리를 익히고, 같은 앱을 PySide6로 확장합니다.',['실제 화면 비교','메모장 만들기','70문제']),(3,'파이썬과 머신러닝','데이터를 준비하고 모델을 학습·평가하며 그래프로 비교합니다.',['분할·전처리','모델·평가','시각 실험']),(4,'파이썬과 컴퓨터 비전','픽셀을 조작하고 이미지 처리에서 객체 검출까지 연결합니다.',['픽셀 실험실','필터·변환','Haar·YOLO'])]:
 home+=f'''<a class="course-card" href="units/unit0{u}/index.html"><div class="course-top"><span class="number">0{u}</span><span class="arrow">↗</span></div><h3>{title}</h3><p>{desc}</p><div class="tags">{''.join(f'<span>{t}</span>' for t in tags)}</div><div class="progress-label"><span>학습 진행</span><span data-unit-progress="{u}" data-total="{len(units[u])}">0%</span></div><progress data-unit-bar="{u}" max="100" value="0" aria-label="{u}단원 진행도"></progress></a>'''
...
def gallery():
 ...
 for id,title,version,desc,example in gallery_items:
  s+=f'<article class="gallery-card"><a href="../../assets/screenshots/{id}.png" target="_blank" rel="noopener"><img src="../../assets/screenshots/{id}.png" loading="lazy" alt="{title} 인사 앱 실제 실행 화면 · 이름 입력창과 인사 및 초기화 버튼"></a><div><h3>{title}</h3><p>{desc}</p><small>{version}</small><button data-example="{example}">코드 열기 ↗</button></div></article>'
...
for u,lessons in units.items():
 ...
 rail=''.join(f'<a href="#{l["id"]}"><span>{i+1:02}</span>{esc(l["title"])}</a>' for i,l in enumerate(lessons))
 body=f'<section class="unit-hero pai-unit-hero">{mascot.image(prefix,"idea" if u==1 else "welcome","eager","pai-unit-image")}<p class="eyebrow">UNIT 0{u} / LEARN BY DOING</p><h1>{title}</h1><p>{design.META[u][2]}</p><div class="tags"><span>교과서 {design.META[u][1]}쪽</span><span>{len(lessons)}개 학습 주제</span></div><div class="actions"><a class="button" href="summary.html">그림으로 정리</a><a class="button primary" href="#lab">실습실 바로가기</a><a class="button" href="#practice">문제 풀기</a><a class="button" href="../../downloads/unit{u}-examples.zip">전체 예제 ZIP ↓</a></div></section><div class="course-layout"><aside class="rail"><p class="eyebrow">IN THIS UNIT</p>{rail}<a href="#lab">코드 실습실</a>'+('<a href="#gallery">GUI 비교 갤러리</a><a href="#simulator">웹 동작 체험</a>' if u==2 else '')+'<a href="#practice">연습 문제</a><a href="#journal">학습 저널</a></aside><main id="main">'
 body+=design.experiments(u)
 for i,l in enumerate(lessons):
  body+=f'<section class="lesson" id="{l["id"]}"><div class="lesson-meta"><span>LESSON {i+1:02}</span><span>{"확장" if l["extra"] else "교과서 연계"} · {l["pages"]}</span></div><h2>{esc(l["title"])}</h2><p class="lesson-lead">{esc(l["lead"])}</p>'+design.visual(u,l)+'<details class="lesson-details"><summary>개념 더 읽기</summary>'+''.join(f'<p>{esc(p)}</p>' for p in l['paragraphs'])+'</details>'
...
sourcebody='<main id="main" class="source-page"><p class="eyebrow">SOURCES & COVERAGE</p><h1>출처와 학습 범위</h1><p>멘토르 「인공지능 파이썬 실무」 PDF와 2026학년도 2학기 교수·학습 및 평가 운영 계획 _g를 바탕으로 작성했습니다. 교과서 개념·예제·문항의 학습 목표를 웹 실습으로 재구성하고, 확장 내용은 표시했습니다. 원본 교과서 PDF는 사이트에 배포하지 않습니다.</p><h2>교과서 대응표</h2><div class="table-wrap"><table><thead><tr><th>단원</th><th>쪽</th><th>학습 내용</th></tr></thead><tbody>'
for u,ls in units.items():
 for l in ls: sourcebody+=f'<tr><td>{u}</td><td>{l["pages"]}</td><td><a href="units/unit0{u}/index.html#{l["id"]}">{l["title"]}</a></td></tr>'
sourcebody+='</tbody></table></div><p><a href="teacher/index.html">교과서 정답·설명 보완 사항은 교사용 요약에 정리했습니다.</a></p><h2>공식 참고 자료</h2><ul>'
