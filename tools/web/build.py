...
# 페이지 공통 스크립트는 layout()에 둡니다(app/auth/follow/sync).
...
board_body = '''<main id="main" class="admin-page teacher-manage"><p class="eyebrow">TEACHER / BOARD</p><h1>반 현황 보드</h1><p class="lead">선택한 반의 참여 현황이 이 자리에 붙습니다. 지금은 반 선택 맥락만 연결합니다.</p><section class="admin-card"><h2>지금 수업하는 반</h2><p class="teacher-class-banner"><strong data-class-label>반을 선택하세요</strong> <span class="small" data-class-id></span></p><p class="small">이후 조회·집계는 이 반만 대상으로 합니다. 여러 반이 동시에 수업해도 데이터가 섞이지 않습니다.</p></section><section class="admin-card teacher-placeholder"><h2>준비 중</h2><p>학생 카드, 주제×학생 히트맵, 문항 분석은 다음 단계에서 이 반(<code data-class-id></code>)을 기준으로 붙습니다.</p></section><p><a href="admin.html">명단 관리로 →</a></p></main>'''
...
(WEB/'teacher/board.html').write_text(teacher_manage('board','반 현황 보드',board_body))
