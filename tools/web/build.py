...
# 교사용 명단 관리. 쓰기 권한은 Firestore 규칙이 admins 문서로 판단합니다.
admin_body = '''<main id="main" class="admin-page"><p class="eyebrow">TEACHER / ROSTER</p><h1>학생 명단 관리</h1><p class="lead">학생의 학번·입학년도·학년·반을 미리 등록합니다. 학생은 이 명단의 값만 받아 가며 스스로 바꿀 수 없습니다.</p><section class="admin-card" id="admin-gate">권한을 확인합니다…</section><div id="admin-tools" hidden><section class="admin-card"><h2>명단 일괄 등록</h2><p class="small">CSV 열: <code>email,studentId,admissionYear,name,grade,classroom,number</code> · 번호는 비워도 됩니다. 머리글은 한글(<code>이메일,학번,입학년도,이름,학년,반,번호</code>)도 인식합니다.</p><label class="file-button">CSV 파일 선택<input type="file" id="roster-file" accept=".csv,text/csv,text/plain"></label><label>또는 직접 붙여넣기<textarea id="roster-text" rows="8" spellcheck="false" placeholder="20314@e-mirim.hs.kr,20314,2025,홍길동,2,3,14"></textarea></label><div class="actions"><button id="roster-check">검사하기</button><button class="primary" id="roster-apply" disabled>명단에 반영</button><button id="roster-sample">예시 채우기</button></div><p id="roster-summary" aria-live="polite"></p><div id="roster-preview" class="admin-table"></div></section><section class="admin-card"><h2>현재 명단 <span id="roster-count" class="pill"></span></h2><div class="actions"><button id="roster-load">불러오기</button><button id="roster-export">CSV 내보내기</button></div><div id="roster-list" class="admin-table"></div></section><section class="admin-card"><h2>미배정 학생</h2><p class="small">로그인했지만 명단에 없는 학교 계정입니다. 학기 중 전학·누락을 여기서 확인해 추가하세요.</p><div class="actions"><button id="unassigned-load">확인하기</button></div><div id="unassigned-list" class="admin-table"></div></section></div><p><a href="index.html">교사용 수업 요약으로 →</a></p></main>'''
(WEB/'teacher').mkdir(exist_ok=True)
(WEB/'teacher/admin.html').write_text(layout('학생 명단 관리',admin_body,'../',0,'<script type="module" src="../assets/admin.js?v=1"></script>'))
